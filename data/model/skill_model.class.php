...
class Skill_Model extends DBObject {
  // Champs BD
  ...
  /* ACCESSEURS */
  public static function get_table_name() { return "skill"; }
  ...
  public static function db_get_select_list( $with_null = false ) {
    ...
    $object_list = Skill_Model::db_get_all();
 ...
 */
  ...
  public function get_player_skill_list($player_id = null) {
    $where = '';
...
FROM `player_skill`
WHERE `skill_id` = '.mysql_ureal_escape_string($this->get_id()).$where;
    $res = mysql_uquery($sql);

    return mysql_fetch_to_array($res);
  }

  public function set_player_skill( $player_id, $experience ) {
    $sql = "REPLACE INTO `player_skill` ( `player_id`, `skill_id`, `experience` ) VALUES (".mysql_ureal_escape_string( $player_id, $this->get_id(), $experience ).")";

    return mysql_uquery($sql);
  }

  public function del_player_skill( $player_id = null ) {
    $where = '';
    if( ! is_null( $player_id )) $where .= '
AND `player_id` = '.mysql_ureal_escape_string($player_id);
